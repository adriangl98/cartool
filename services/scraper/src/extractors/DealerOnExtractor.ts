 ...
 * next-page links.
 */
export class DealerOnExtractor extends BaseExtractor {
  // -------------------------------------------------------------------------
  // Core extraction
  // -------------------------------------------------------------------------
  ...
  private async extractDataAttributes(
    page: Page,
  ): Promise<DealerOnRawItem[]> {
    return page.$$eval("[data-vin]", (elements) =>
      elements.map((el) => ({
  ...
  private mapToRawListings(
    ...
    for (const item of items) {
      ...
      const special =
        specials.get(vin) ?? this.findSpecialByModel(specials, model);
      if (special) {
        ...
        if (
          special.leaseTermMonths !== undefined ||
  ...
  private async extractSpecials(
    page: Page,
  ): Promise<Map<string, SpecialsData>> {
    ...
    for (const selector of SPECIALS_SELECTORS) {
      ...
      for (const card of cards) {
        const text: string = await card.evaluate(
          (el) => el.textContent ?? "",
        );
