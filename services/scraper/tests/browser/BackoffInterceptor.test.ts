import { withRetry, RetryableError } from "../../src/browser/BackoffInterceptor";
...
describe("withRetry", () => {
  ...
  function advanceTimersByPromise(ms: number): Promise<void> {
  ...
  it("throws non-retryable errors immediately without retry", async () => {
    ...
    await expect(withRetry(fn)).rejects.toThrow("Navigation timeout");
    expect(fn).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries on HTTP 429 errors with exponential back-off", async () => {
    const fn = jest
      .fn()
      .mockRejectedValueOnce(new RetryableError("429 Too Many Requests", 429))
      .mockRejectedValueOnce(new RetryableError("429 Too Many Requests", 429))
      .mockRejectedValueOnce(new RetryableError("429 Too Many Requests", 429))
  ...
  it("throws after exhausting all retries", async () => {
    ...
    const fn = jest
      .fn()
      .mockRejectedValue(new RetryableError("429", 429));
  ...
  it("uses default base delay of 8000ms", async () => {
    ...
    jest.spyOn(console, "log").mockImplementation((...args: unknown[]) => {
      try {
        ...
        if (parsed.delayMs) delays.push(parsed.delayMs);
      } catch {
        // ignore
      }
    });
  ...
  it("fires back-off retries at 8s, 16s, 32s with default options and then throws", async () => {
    ...
    const fn = jest
      .fn()
      .mockRejectedValue(new RetryableError("429 Too Many Requests", 429));
    ...
    global.setTimeout = ((cb: () => void) => origSetTimeout(cb, 0)) as unknown as typeof setTimeout;
