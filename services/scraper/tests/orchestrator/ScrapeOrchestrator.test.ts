import { ScrapeOrchestrator } from "../../src/orchestrator/ScrapeOrchestrator";
...
const mockQueue = { upsertJobScheduler: mockUpsertJobScheduler } as any;
...
function upsertCallsForDealer(dealerId: string) {
  return (mockUpsertJobScheduler.mock.calls as Array<[string, unknown, { data: ScrapeJobPayload }]>)
...
describe("ScrapeOrchestrator", () => {
  ...
  it("queries only active dealers on start()", async () => {
    const orchestrator = new ScrapeOrchestrator(mockPool, mockQueue);
  ...
  it("schedules one inventory job per dealer with a 6-hour repeat", async () => {
    ...
    const inventoryCalls = (mockUpsertJobScheduler.mock.calls as Array<[string, { every?: number }, unknown]>)
  ...
  it("sets jobType=inventory and correct url on inventory jobs", async () => {
    ...
    const calls = upsertCallsForDealer(DEALER_WITH_SPECIALS.id).filter(([id]) =>
  ...
  it("configures inventory jobs with 3 attempts and exponential back-off at 8s", async () => {
    ...
    const [, , jobTemplate] = (mockUpsertJobScheduler.mock.calls as Array<[string, unknown, { opts: { attempts: number; backoff: { type: string; delay: number } } }]>)
  ...
  it("schedules one specials job for a dealer that has specials_url", async () => {
    ...
    const specialsCalls = (mockUpsertJobScheduler.mock.calls as Array<[string, unknown, unknown]>)
  ...
  it("schedules specials job with 12-hour repeat and jobType=specials", async () => {
    ...
    const [, repeatOpts, jobTemplate] = (mockUpsertJobScheduler.mock.calls as Array<[string, { every?: number }, { data: ScrapeJobPayload }]>)
  ...
  it("schedules one buy_rates job per dealer with a monthly cron pattern", async () => {
    ...
    const buyRatesCalls = (mockUpsertJobScheduler.mock.calls as Array<[string, { pattern?: string }, { data: ScrapeJobPayload }]>)
