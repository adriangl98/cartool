...
exports.ScrapeOrchestrator = void 0;
...
class ScrapeOrchestrator {
    ...
    async start() {
        const result = await this.pool.query("SELECT id, platform, inventory_url, specials_url FROM dealers WHERE is_active = TRUE");
        let scheduled = 0;
        for (const dealer of result.rows) {
            // Inventory — every 6 hours
            await this.queue.upsertJobScheduler(`inventory:${dealer.id}`, { every: SIX_HOURS_MS }, {
                name: "scrape-job",
                data: {
                    dealerId: dealer.id,
            ...
            if (dealer.specials_url) {
                await this.queue.upsertJobScheduler(`specials:${dealer.id}`, { every: TWELVE_HOURS_MS }, {
                    name: "scrape-job",
                    data: {
                        dealerId: dealer.id,
            ...
            await this.queue.upsertJobScheduler(`buy_rates:${dealer.id}`, { pattern: BUY_RATES_CRON }, {
                name: "scrape-job",
                data: {
                    dealerId: dealer.id,
                    url: dealer.inventory_url,
        ...
        console.log(JSON.stringify({
            ...
            queueName: shared_1.QUEUE_NAMES.SCRAPE,
        }));
    }
}
exports.ScrapeOrchestrator = ScrapeOrchestrator;
