import { AddonDetector } from "../../src/normalizer/AddonDetector";
import type { DetectedAddon } from "../../src/types/DetectedAddon";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function findAddon(addons: DetectedAddon[], name: string): DetectedAddon | undefined {
...
describe("AddonDetector", () => {
  let detector: AddonDetector;

  beforeEach(() => {
    detector = new AddonDetector();
  });
  ...
  it("falls back to midpoint cost when no dollar amount is near the keyword", () => {
    const result = detector.detect(
      "All vehicles include nitrogen-filled tires as standard equipment.",
    );
  ...
  it("returns multiple DetectedAddon records when multiple groups match", () => {
    const result = detector.detect(
      "All vehicles include nitrogen-filled tires and interior protection.",
    );
  ...
  it("detects Generic Package with explicit cost when dollar amount present", () => {
    const result = detector.detect(
      "Includes dealer installed accessories package ($1,200).",
    );
  ...
  it("returns Generic Package with detectedCost = undefined when no dollar amount found", () => {
    const result = detector.detect(
      "All vehicles include dealer installed accessories.",
    );
    const addon = findAddon(result, "Generic Package");
    expect(addon).toBeDefined();
  ...
  it("matches each keyword group at most once even when multiple triggers appear", () => {
    const result = detector.detect(
      "window tint and tinted windows and window film are all applied.",
    );
