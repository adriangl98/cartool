 ...
 */
const ADDON_GROUPS: ReadonlyArray<{
 ...
 */
export class AddonDetector {
  detect(text: string): DetectedAddon[] {
    // ---------------------------------------------------------------------------
    // Pass 1 — find all matching positions across all groups
    // ---------------------------------------------------------------------------
    interface MatchInfo {
      groupIndex: number;
      matchStart: number;
    ...
    for (let i = 0; i < allMatches.length; i++) {
      ...
      const forwardEnd = Math.min(
        info.matchEnd + CONTEXT_RADIUS,
        nextMatchStart,
      );
      const forwardContext = text.slice(info.matchEnd, forwardEnd);

      let detectedCost: number | undefined;
