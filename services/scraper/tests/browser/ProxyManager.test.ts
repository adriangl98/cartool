import { ProxyManager } from "../../src/browser/ProxyManager";
import type { Proxy } from "../../src/browser/ProxyManager";
...
describe("ProxyManager", () => {
  ...
  describe("parseProxyList", () => {
    it("parses host:port:user:pass lines correctly", () => {
      const proxies = ProxyManager.parseProxyList(THREE_PROXIES);
    ...
    it("handles passwords containing colons", () => {
      const proxies = ProxyManager.parseProxyList(
        "host.example.com:9090:admin:p@ss:word:complex"
      );
    ...
    it("ignores empty lines and whitespace-only lines", () => {
      const raw = "\n  \nproxy1.example.com:8080:user1:pass1\n\n  \n";
      const proxies = ProxyManager.parseProxyList(raw);
    ...
    it("throws on entries with fewer than 4 colon-separated parts", () => {
      expect(() =>
        ProxyManager.parseProxyList("bad-entry:8080")
      ).toThrow("invalid proxy entry");
    });

    it("throws on invalid port numbers", () => {
      expect(() =>
        ProxyManager.parseProxyList("host:notaport:user:pass")
  ...
  describe("constructor", () => {
    it("throws when proxy list is empty string", () => {
      expect(
        () => new ProxyManager(redis as never, "")
    ...
    it("reports correct pool size", () => {
      const pm = new ProxyManager(redis as never, THREE_PROXIES);
  ...
  describe("getProxy", () => {
    ...
    it("allows reuse of the same proxy for different dealer domains", async () => {
      ...
      const pm = new ProxyManager(
        redis as never,
        "solo.example.com:8080:user1:pass1"
      );
    ...
    it("never returns the same proxy for the same domain within cooldown (20 requests)", async () => {
      const pm = new ProxyManager(redis as never, FIVE_PROXIES);
    ...
    it("simulates 20 requests across multiple domains without IP reuse per domain", async () => {
      ...
      const usedPerDomain = new Map<string, Set<string>>();
    ...
    it("throws when all proxies are on cooldown for the requested domain", async () => {
      ...
      await expect(pm.getProxy(domain)).rejects.toThrow(
        'no eligible proxy available for domain "samesnissan.com"'
      );
    });
  ...
  describe("reportFailure / reportSuccess", () => {
    it("excludes a proxy after 3 consecutive failures", async () => {
      ...
      redis.clear();
      const pm2 = new ProxyManager(redis as never, THREE_PROXIES);
