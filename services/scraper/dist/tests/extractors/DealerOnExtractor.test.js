...
const DealerOnExtractor_1 = require("../../src/extractors/DealerOnExtractor");
...
function extractDataVinItems(html) {
    const regex = /<div[^>]+data-vin="([^"]*)"[^>]*>/gi;
    const items = [];
    let match;
...
function setupPageMocks(html) {
    ...
    mock$$eval.mockImplementation((selector, _fn) => {
...
function createExtractor() {
    return new DealerOnExtractor_1.DealerOnExtractor({
...
describe("DealerOnExtractor", () => {
    ...
    describe("single-page extraction", () => {
        ...
        it("maps data attributes correctly", async () => {
            ...
            const extractor = createExtractor();
            const listings = await extractor.run();
            const silverado = listings.find((l) => l.vin === "1GCUYEED5RZ100001");
        ...
        it("sets undefined for missing optional fields (not null or 0)", async () => {
            ...
            const traverse = listings.find((l) => l.vin === "KL79MPS26RB400004");
    ...
    describe("acceptance criteria — attribute name compliance", () => {
        ...
        it("parses data-price, data-vin, and data-msrp from DealerOn fixture HTML via $$eval", async () => {
            ...
            const extractor = createExtractor();
            await extractor.run();
            expect(mock$$eval).toHaveBeenCalledWith("[data-vin]", expect.any(Function));
        });
    ...
    describe("S3 archival", () => {
        ...
        it("sets rawS3Key on every listing", async () => {
            ...
            for (const listing of listings) {
                ...
                expect(listing.rawS3Key).toMatch(/^dealers\/test-dealer-uuid-002\/\d+\.html$/);
            }
        });
    ...
    describe("specials fine-print extraction", () => {
        it("extracts lease terms from specials cards matched by VIN", async () => {
            ...
            const extractor = createExtractor();
            const listings = await extractor.run();
            const silverado = listings.find((l) => l.vin === "1GCUYEED5RZ777771");
            expect(silverado).toBeDefined();
        ...
        it("matches specials by model name when VIN not in card", async () => {
            ...
            const extractor = createExtractor();
            const listings = await extractor.run();
            const equinox = listings.find((l) => l.vin === "3GNAXKEV1RS888882");
        ...
        it("leaves non-matching vehicles without specials data", async () => {
            ...
            const extractor = createExtractor();
            const listings = await extractor.run();
            const traverse = listings.find((l) => l.vin === "KL79MPS26RB999993");
            expect(traverse).toBeDefined();
            expect(traverse.advertisedMonthly).toBeUndefined();
    ...
    describe("pagination", () => {
        ...
        it("uses HumanBehavior.randomMousePath before clicking Load More", async () => {
            ...
            const extractor = createExtractor();
            await extractor.run();
            expect(HumanBehavior_1.HumanBehavior.randomMousePath).toHaveBeenCalledWith(mockPage, 460, // x + width/2
            920);
        });
    });
    // -------------------------------------------------------------------------
