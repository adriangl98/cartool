...
function createMockPage(scrollHeight = 1000, innerHeight = 600) {
...
describe("HumanBehavior", () => {
    describe("randomScroll", () => {
        it("scrolls in multiple incremental steps, not a single jump", async () => {
            ...
            const scrollCalls = page.evaluate.mock.calls.slice(2);
            expect(scrollCalls.length).toBeGreaterThanOrEqual(2);
        });
        it("adds a delay between each scroll step", async () => {
            const page = createMockPage(1200, 600);
            await HumanBehavior_1.HumanBehavior.randomScroll(page);
            const waitCalls = page.waitForTimeout.mock.calls;
        ...
        it("does not scroll when content fits within viewport", async () => {
            ...
            expect(page.evaluate.mock.calls.length).toBe(2);
        });
    });
    describe("randomMousePath", () => {
        it("moves mouse through multiple intermediate points before clicking", async () => {
            ...
            page.evaluate.mockResolvedValue({ x: 0, y: 0 });
            await HumanBehavior_1.HumanBehavior.randomMousePath(page, 500, 500);
        ...
        it("clicks the target coordinates after the mouse path", async () => {
            const page = createMockPage();
            page.evaluate.mockResolvedValue({ x: 100, y: 100 });
            await HumanBehavior_1.HumanBehavior.randomMousePath(page, 300, 400);
        ...
        it("adds delays between mouse movements", async () => {
            const page = createMockPage();
            page.evaluate.mockResolvedValue({ x: 0, y: 0 });
            await HumanBehavior_1.HumanBehavior.randomMousePath(page, 200, 200);
