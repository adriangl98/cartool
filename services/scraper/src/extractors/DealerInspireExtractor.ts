...
// ---------------------------------------------------------------------------
// Dealer Inspire API feed assumed types (validate during integration testing)
// ---------------------------------------------------------------------------

interface DealerInspireApiResponse {
  inventory: DealerInspireApiVehicle[];
  total_count?: number;
  page?: number;
  page_size?: number;
}

interface DealerInspireApiVehicle {
  vin: string;
  year: number;
  make: string;
  model: string;
 ...
 */
export class DealerInspireExtractor extends BaseExtractor {
  ...
  private async extractFromApiFeed(responses: string[]): Promise<RawListing[]> {
    const listings: RawListing[] = [];

    for (const raw of responses) {
      let parsed: DealerInspireApiResponse;
      try {
        parsed = JSON.parse(raw) as DealerInspireApiResponse;
      } catch {
        continue; // Skip malformed JSON
      }

      if (!Array.isArray(parsed.inventory)) continue;
  ...
  private mapVehicleToListing(
    ...
    if (v.payments?.finance) {
      ...
      if (
        listing.advertisedMonthly === undefined &&
  ...
  private async extractFromHtmlAttributes(page: Page): Promise<RawListing[]> {
    ...
    for (const item of items) {
      ...
      listings.push({
        ...
        msrp,
        sellingPrice:
          sellingPrice !== undefined && !isNaN(sellingPrice) && sellingPrice > 0
            ? sellingPrice
            : undefined,
        transactionType: "finance",
        scrapedAt: new Date(),
