...
import { SincroExtractor } from "../../src/extractors/SincroExtractor";
...
function loadJsonFixture(): string {
  return loadFixture("price-stack-response.json");
}

function createExtractor(): SincroExtractor {
  const extractor = new SincroExtractor({
 ...
 */
async function simulateXhrIntercept(
  url: string,
  jsonBody: string,
...
function setupFallbackPageMocks(html: string) {
  ...
  const vinRegex =
    /data-vin="([^"]+)"\s+data-year="([^"]+)"\s+data-make="([^"]+)"\s+data-model="([^"]+)"\s+data-trim="([^"]*)"\s+data-msrp="([^"]+)"\s+data-price="([^"]*)"/g;
  const items: Array<{
    vin: string;
    year: string;
    make: string;
...
function setupCookieConsentMocks(html: string) {
...
describe("SincroExtractor", () => {
  ...
  describe("XHR intercept path", () => {
    it("captures price stack and returns correct number of listings", async () => {
      ...
      mockGoto.mockImplementation(async () => {
        await simulateXhrIntercept(
          "https://www.toyotaoflaredo.com/SearchNew/GetVehicles",
          jsonBody
        );
      });
    ...
    it("populates all required RawListing fields", async () => {
      ...
      mockGoto.mockImplementation(async () => {
        await simulateXhrIntercept(
          "https://cdn.sincro.com/SearchNew",
          jsonBody
        );
      });
    ...
    it("maps lease fields correctly and sets transactionType to 'lease'", async () => {
      ...
      mockGoto.mockImplementation(async () => {
        await simulateXhrIntercept(
          "https://www.toyotaoflaredo.com/SearchNew",
          jsonBody
        );
      });
      ...
      const tundra = listings.find(
        (l) => l.vin === "5TFBY5F18NX901234"
      ) as RawListing;
    ...
    it("maps finance fields correctly and keeps transactionType as 'finance'", async () => {
      ...
      const camry = listings.find(
        (l) => l.vin === "JTDKN3DU5R0567890"
      ) as RawListing;
    ...
    it("sets transactionType 'lease' when vehicle has both lease and finance", async () => {
      ...
      const runner = listings.find(
        (l) => l.vin === "5TDZA23C06S789012"
      ) as RawListing;
    ...
    it("defaults transactionType to 'finance' when no payment data", async () => {
      ...
      const rav4 = listings.find(
        (l) => l.vin === "JTERU5JR7R5234567"
      ) as RawListing;
    ...
    it("uploads raw JSON to S3 with .json key", async () => {
      ...
      const jsonUploadCall = mockUpload.mock.calls.find(
        ([key]: [string]) => key.endsWith(".json")
      );
      expect(jsonUploadCall).toBeDefined();
      expect(jsonUploadCall![0]).toMatch(
        /^dealers\/test-dealer-toyota-001\/\d+\.json$/
      );
  ...
  describe("pagination", () => {
    it("follows next-page links and combines results", async () => {
      ...
      const htmlUploads = mockUpload.mock.calls.filter(([key]: [string]) =>
        key.endsWith(".html")
      );
