...
const NormalizationService_1 = require("../../src/normalizer/NormalizationService");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
describe("NormalizationService", () => {
    let service;
    beforeEach(() => {
        service = new NormalizationService_1.NormalizationService();
    });
    // -------------------------------------------------------------------------
    ...
    describe("adjustedSellingPrice — alias coverage", () => {
        ...
        test.each(cases)('maps "$alias" alias to adjustedSellingPrice', ({ sellingPrice }) => {
    ...
    describe("rebate detection", () => {
        ...
        it('detects "includes $X rebate" keyword and captures embedded amount', () => {
            const listing = makeListing({
                sellingPrice: 26000,
                rawFinePrintText: "Special offer includes $1,500 rebate for qualified buyers.",
            });
            const result = service.normalize(listing);
            expect(result.rebateDetected).toBe(true);
    ...
    describe("addonAdjustedPrice and detectedAddons", () => {
        ...
        it("computes addonAdjustedPrice = adjustedSellingPrice + detected add-on costs", () => {
            ...
            const listing = makeListing({
                sellingPrice: 25000,
                rawFinePrintText: "Includes nitrogen-filled tires and window tint ($699)",
            });
            const result = service.normalize(listing);
        ...
        it("excludes Generic Package with no cost from addonAdjustedPrice sum", () => {
            ...
            const result = service.normalize(listing);
            const genericAddon = result.detectedAddons.find((a) => a.addonName === "Generic Package");
